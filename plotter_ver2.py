...
def plot_variable(dfs, variable, bins, xlabel, luminosity, out_path):
    """
    dfs: {sample_name: (df, label, is_data)}
         df는 object definition + selection이 이미 다 적용된 RDataFrame이어야 함
    """
    hep.style.use("CMS")
    ...
    for key, (df, label, is_data) in dfs.items():
        ...
        if is_data:
            hist = df.Histo1D(hist_model, variable).GetValue()
        else:
            df = df.Define("weight", f"{luminosity} * xsecWeight * Pileup_weight * Muon_weight * PUJetID_weight")
            hist = df.Histo1D(hist_model, variable, "weight").GetValue()
        hist.SetDirectory(0)
...
if __name__ == "__main__":
    ...
    variables = [
        ("GoodMuon_pt[0]", (20, 0, 100), r"Muon $p_T$ [GeV]"),
        ("MET_pt", (40, 0, 200), r"MET [GeV]"),
    ]

    for variable, bins, xlabel in variables:
        out_name = f"{variable.split('[')[0]}_{selection}.pdf"
